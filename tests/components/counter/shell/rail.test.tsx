...
describe("Rail", () => {
  ...
  /*
   * Eighteen is the DEVELOPER's rail. `/dashboard/admin/**` redirects anyone
   * who is not one to `/dashboard/forbidden`, so Monitoring is drawn for a
   * developer and for nobody else — this used to assert every item for every
   * reader, which is the rail an owner was actually shown while the gate
   * behind it refused them.
   */
  it("renders all eighteen destinations for a developer", () => {
  ...
  it("lights Overview alone on /dashboard, not all eighteen", () => {
    render(<Rail pathname="/dashboard" />)
    const current = screen
      .getAllByRole("link")
      .filter((l) => l.getAttribute("aria-current") === "page")
