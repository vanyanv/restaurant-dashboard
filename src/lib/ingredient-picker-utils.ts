 ...
 *
 * Moved here from `src/app/dashboard/(editorial)/recipes/components/` when the
 * Counter rebuild retired that route. It was never a component: it is three
 * pure functions and a type, and it was imported by the price monitor, the
 * ingredient-audit log and a server action — none of which is a recipe page.
 * Living inside a page directory is what made deleting that page break three
 * unrelated surfaces.
 *
 * The hex literals below are the pre-Counter palette and stay hex on purpose:
 * they are consumed by editorial surfaces that have not been rebuilt, and
 * `src/lib` is outside the Counter token rules. A Counter surface that needs a
 * category swatch takes it from `ct-` tokens instead.
 */
