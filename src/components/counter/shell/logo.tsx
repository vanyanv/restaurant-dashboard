 ...
 *
 * There used to be a `Wordmark` beside this that set the name in the display
 * face instead. It was written because `Rail` recorded "we have no logo asset"
 * — and that was never true; `public/logo.png` has been in the tree since the
 * first commit. Every slot it stood in now draws the mark, so it is gone. Note
 * 15 is the reason the difference matters: "the wordmark is the palette's
 * alibi", the red and the signal yellow read as a designer's choice until the
 * mark sits next to them, and an alibi has to be the real thing.
 *
