 ...
 * no longer on it.
 *
 * ## What this page has no control for, and why that is carried work
 *
 * The store selector and the date control are the phone SHELL's, not the
 * page's: the prototype puts them in `.mtop`, which sits outside `.mscroll` and
 * therefore outside the fidelity surface entirely, next to `.mtabs`. Neither
 * exists on Counter yet. So this page reads `?range` / `?store` / `?cmp` — the
 * same three parameters the desk writes, so a redirect from `/dashboard?range=d7`
 * lands on the same window here — and offers no way to change them from the
 * phone. The editorial `MToolbar` it replaces did offer one, in a different
 * vocabulary (`?period=today`), and driving Counter's range from that would put
 * two range vocabularies on one page, which is note 60's defect exactly. The
 * phone shell task takes `.mtop`; this is recorded so it is not lost.
 */
