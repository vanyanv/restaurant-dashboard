...
export function DashboardContent({ 
  ...
  const getManagerCount = () => {
    return initialStores.reduce((total, store) => total + store._count.managers, 0)
  }
        ...
        <div className="grid auto-rows-min gap-4 md:grid-cols-4">
          ...
          <div className="rounded-xl border bg-card text-card-foreground shadow p-6">
            ...
            <div className="text-2xl font-bold">
              {initialAnalytics ? formatCurrency(initialAnalytics.totalRevenue) : "$0"}
            </div>
            <p className="text-xs text-muted-foreground flex items-center gap-1">
              {initialAnalytics && getTrendIcon(initialAnalytics.trends.revenueGrowth)}
              {initialAnalytics ? getTrendText(initialAnalytics.trends.revenueGrowth) : "No data"}
            </p>
          </div>
          ...
          <div className="rounded-xl border bg-card text-card-foreground shadow p-6">
            ...
            <div className="text-2xl font-bold">
              {initialAnalytics ? `${initialAnalytics.avgPrepCompletion}%` : "0%"}
            </div>
            <p className="text-xs text-muted-foreground">
              {initialAnalytics ? `From ${initialAnalytics.totalReports} reports` : "No reports"}
            </p>
          </div>
        </div>
